...
class ColaDePrioridadmax:
    ...
    def __init__(self):
        ''''lista de tuplas ya que el algoritmo de dijkstra lo requiere'''
        self._lista=[(0,0)] 
        self._tamanio=0
    ...
    def esta_vacia(self):
        '''si la cola no tiene elementos retorna FALSE'''
    ...
    def hijo_max(self, i):
        if i*2+1 > self._tamanio:
            ...
            if self._lista[i*2][0] > self._lista[i*2 +1][0]:
                return i*2
            else:
                return i*2+1
    ...
    def eliminar_max (self): #simpre elimina la raiz
    ...
    def devolver_maximo(self):
        return self.lista[1][1]
    
    #recorre la lista e intercambia la clave de un nodo por otra, infiltrando hacia arriba si es necesario
    def decrementar_clave (self, valor, nuevaclave): #valor=vertice
        ...
        while not hecho and i<=self._tamanio:
            if self._lista[i][1] == valor:
                ...
                i = i + 1
        if clave >0:
            self._lista[clave]= (nuevaclave, self._lista[clave][1])
